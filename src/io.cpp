...
Value read_io(const std::string& path) {
    ...
    GblnErrorCode result = gbln_read_io(path.c_str(), &out_value);
    ...
    if (result != Ok) {
        ...
        throw IoError("Failed to read I/O file '" + path + "': " + msg);
    }
    
    if (!out_value) {
        throw IoError("I/O read returned null pointer for: " + path);
    }
...
Value read_io(const std::filesystem::path& path) {
    return read_io(path.string());
}

void write_io(const Value& value, const std::string& path, const Config& config) {
    // Validate config
    config.validate();
